...
(async () => {
 ...
 ${last?`<section class="panel pad latest-workout"><div class="section-title"><h2>Последняя тренировка</h2><button class="btn ghost small" data-nav="history">Вся история ${ic('arrow')}</button></div><div class="row between"><div><h3>${h(last.title)}</h3><p class="help">${dateLabel(last.date)} · ${last.exercises.length} упражнений · ${N.totals(last).done} отмеченных подходов</p></div><button class="btn outline small" data-action="report" data-id="${h(last.id)}">${ic('copy')}Отчёт для тренера</button></div></section>`:''}`;
    ...
    function openPicker(c, replacing = false, mode = '') {
        ...
        const replacingEntered = !!(replacing && old?.logs?.some(N.entered));
        ui.picker = { ...c, replacing, mode, replacingEntered };
        ui.search = '';
        const opts = replacing ? N.alternatives(old, state.profile, mode || 'movement') : N.library();
        ui.picker.options = opts;
        showModal(replacing ? 'Выбрать замену' : 'Добавить упражнение', `<div class="modal-body">${replacing ? `<div class="notice">${ic('swap')}<div>Вместо <strong>${h(old.name)}</strong>. ${mode === 'muscle' ? 'Совпадает основная мышечная группа; движение может отличаться.' : 'Сохранён тип движения, насколько позволяет оборудование.'} Рабочий вес не переносится.${replacingEntered ? '<br><b>В старом упражнении есть введённые данные. При подтверждении замены будут удалены только эти незавершённые результаты, а новое упражнение останется на той же позиции.</b>' : ''}</div></div>` : '<p class="help" style="margin-bottom:15px">Можно добавить упражнение из каталога или своё. По умолчанию во время занятия — только на сегодня.</p>'}<label>Поиск<input id="exercise-search" type="search" placeholder="Название, синоним или мышца" autocomplete="off"></label><div class="search-results" id="exercise-options">${pickerOptions(opts)}</div><div class="divider"></div><button class="btn outline full" data-action="custom-ex">${ic('plus')}Написать своё упражнение</button></div>`);
    ...
    function saveExercise(form) { const c = ui.editor, list = listFor(c), f = new FormData(form); if (!list)
        ...
        throw Error('Нельзя менять структуру упражнения с результатами. Добавь упражнение отдельно.'); const targetText = String(f.get('targets')).trim(), suppliedTargets = targetText ? targetText.split(/[,;]/).map(x => x.trim()) : []; const targets = N.syncExerciseTargets(c.exercise, e.sets, e.repsMin, e.repsMax, suppliedTargets); if (!targets.every(x => /^\d{1,3}(?:[–-]\d{1,3})?$/.test(x.reps) && x.reps.split(/[–-]/).every(y => Number(y) > 0 && Number(y) <= 500)))
        throw Error('В плане подходов используй число или диапазон повторов.'); e.targets = targets; if (!e.originalName)
    ...
    async function actions(b) {
        ...
        switch (a) {
            ...
            case 'move-ex':
                ...
                break;
            case 'delete-workout-ex': {
                const session = state.draft, index = session?.exercises.findIndex(e => e.id === id);
                if (!session || index == null || index < 0) break;
                confirm('Удалить упражнение из текущей тренировки?', 'Будут удалены только это упражнение и его незавершённые результаты. Шаблон программы и история не изменятся.', () => { session.exercises.splice(index, 1); changed(); render(); }, 'Удалить');
                break;
            }
            case 'delete-ex': {
            ...
            case 'add-set': {
                ...
                const target = N.targetLabel(e.repsMin, e.repsMax);
                e.sets += 1;
                e.targets = N.syncExerciseTargets(e, e.sets, e.repsMin, e.repsMax, [...(e.targets || []).map(x => x.reps), target]);
                for (const side of e.unilateral ? ['left', 'right'] : ['both'])
                    e.logs.push({ id: uid(), number: n, side, weight: '', reps: '', rir: '', done: false, measurement: e.measurement || 'reps', target });
                changed();
                render();
                break;
            }
