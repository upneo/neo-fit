...
(() => {
    ...
    let ui = { tab: 'today', program: 0, mode: config.supabaseUrl && config.publishableKey ? 'cloud' : 'local', register: false, date: N.dateKey(), month: N.dateKey().slice(0, 7), editor: null, picker: null, historyEdit: null, search: '', group: '' };
 ...
 setStatus('Сохраняем изменения…');clearTimeout(saveTimer);saveTimer=setTimeout(persistNow,180);
    ...
    function goto(tab) { if(!navs.some(n=>n[0]===tab))return; ui.tab=tab; history.replaceState(null,'','#'+tab); render(); window.scrollTo(0,0); }
    function renderAuth() {
  ...
  $('#root').innerHTML = `<div class="auth-shell">
    ...
    async function authenticate(form) { const f = new FormData(form), name = String(f.get('username')).trim(), password = String(f.get('password')); if (ui.register && password.length < 8)
        ...
        doc = await store.login(name, password); state = doc; lastSavedRevision = store.pending ? -1 : state.revision; if (!state.profile.completed && !state.trackers.length && !state.intakes.length)
        state.profile.timeZone = Intl.DateTimeFormat().resolvedOptions().timeZone || state.profile.timeZone; ui.tab = ui.register ? 'profile' : 'today'; ui.program = state.schedule.nextIndex; ui.date = today(); ui.month = ui.date.slice(0, 7); setStatus(store.mode === 'cloud' ? (store.conflict ? 'Конфликт версий — открой профиль' : store.pending ? 'Есть несинхронизированные изменения' : 'Загружено из облака') : 'Сохранено на устройстве', !!store.conflict); render(); if (store.mode === 'local' && browserStorage.ephemeral) {
  ...
  ${({today:renderToday,program:renderProgram,workout:renderWorkout,history:renderHistory,supplements:()=>renderTrackers('supplement'),peptides:()=>renderTrackers('peptide'),nutrition:renderNutrition,profile:renderProfile}[ui.tab])()}
 ...
 <p class="help">${state.schedule.mode==='rotation'?'Очередь продолжается, даже когда меняется неделя.':'Программа закреплена за днями недели.'}</p></section></div>
 ...
 ${last?`<section class="panel pad latest-workout"><div class="section-title"><h2>Последняя тренировка</h2><button class="btn ghost small" data-nav="history">Вся история ${ic('arrow')}</button></div><div class="row between"><div><h3>${h(last.title)}</h3><p class="help">${dateLabel(last.date)} · ${last.exercises.length} упражнений · ${N.totals(last).done} отмеченных подходов</p></div><button class="btn outline small" data-action="report" data-id="${h(last.id)}">${ic('copy')}Отчёт для тренера</button></div></section>`:''}`;
    ...
    function showTechnique(kind, id) { const e = listFor(exContext(kind, id))?.find(x => x.id === id); if (!e)
        return; const q = encodeURIComponent(e.name + ' техника выполнения'); showModal('Техника: ' + e.name, `<div class="modal-body"><span class="pill">Краткая памятка</span><p style="margin:17px 0;font-size:14px;line-height:1.8">${h(e.technique || 'Для своего упражнения добавь памятку в редакторе. Проверяй настройку оборудования с тренером.')}</p><div class="notice warn">Описание общее и не учитывает твою анатомию и модель тренажёра. При боли останови упражнение. Поисковые результаты не проходят проверку приложения.</div><div class="row"><a class="btn primary" href="https://www.youtube.com/results?search_query=${q}" target="_blank" rel="noopener noreferrer">${ic('play')}Найти на YouTube ↗</a><a class="btn outline" href="https://yandex.ru/search/?text=${q}" target="_blank" rel="noopener noreferrer">${ic('search')}Найти в Яндексе ↗</a></div><p class="help" style="margin-top:18px">Это именно поиск по названию, а не ссылка на заранее проверенный ролик. Общие памятки составлены для интерфейса, они не являются частью PDF katuabu.</p></div>`); }
    function openPicker(c, replacing = false) { const old = listFor(c)?.find(e => e.id === c.id); if (replacing && old?.logs?.some(N.entered)) {
        toast('В упражнении уже есть результаты. Добавь замену отдельным упражнением, чтобы не потерять сделанное.', true);
        return;
    } ui.picker = { ...c, replacing }; ui.search = ''; const opts = replacing ? N.alternatives(old, state.profile) : N.library(); ui.picker.options = opts; showModal(replacing ? 'Выбрать замену' : 'Добавить упражнение', `<div class="modal-body">${replacing ? `<div class="notice">${ic('swap')}<div>Вместо <strong>${h(old.name)}</strong>. Похожие по движению, с учётом оборудования и исключений. Рабочий вес не переносится.</div></div>` : '<p class="help" style="margin-bottom:15px">Можно добавить пресс, упражнение из каталога или своё. По умолчанию во время занятия — только на сегодня.</p>'}<label>Поиск<input id="exercise-search" type="search" placeholder="Название или группа мышц" autocomplete="off"></label><div class="search-results" id="exercise-options">${pickerOptions(opts)}</div><div class="divider"></div><button class="btn outline full" data-action="custom-ex">${ic('plus')}Написать своё упражнение</button></div>`); $('#exercise-search').focus(); }
    function pickerOptions(opts) { return opts.length ? opts.map(e => `<button class="selection-card" data-action="pick-ex" data-catalog="${h(e.catalogId)}"><span class="tracker-icon">${ic('dumbbell')}</span><span class="grow"><strong>${h(e.name)}</strong><small>${h(e.group)} · ${h(N.equipment[e.equipment])}</small></span>${ic('arrow')}</button>`).join('') : '<p class="help" style="padding:20px 0">Точного аналога для выбранного оборудования нет. Добавь своё упражнение, не считая любое движение на ту же мышцу равноценной заменой.</p>'; }
    ...
    function editSchedule() { formModal('Расписание тренировок', `<div class="form-grid"><div class="form-full">${select('Режим', 'mode', state.schedule.mode, { rotation: 'По очереди, без сброса недели', weekly: 'Закрепить шаблоны за днями' })}</div><div class="form-full"><div class="field-label" style="margin-bottom:9px">Дни недели</div>${daySelect('weekdays', state.schedule.weekdays)}</div>${state.programs.length ? `<div class="form-full">${select('Следующий шаблон в очереди', 'nextIndex', state.schedule.nextIndex, Object.fromEntries(state.programs.map((p, i) => [i, p.title])))}</div><div class="form-full"><p class="help">Ниже — соответствие для режима «по дням». В режиме очереди эти поля не используются.</p></div>${order.map(d => select(days[d], 'map_' + d, state.schedule.map[d] ?? 0, Object.fromEntries(state.programs.map((p, i) => [i, p.title])))).join('')}` : ''}</div>`, form => { const f = new FormData(form), chosen = f.getAll('weekdays').map(Number); if (chosen.length < 1)
        throw Error('Выбери хотя бы один день'); const map = {}; order.forEach(d => map[d] = Number(f.get('map_' + d) || 0)); state.schedule = { mode: String(f.get('mode')), weekdays: chosen, map, nextIndex: Number(f.get('nextIndex') || 0) }; changed(); close(); render(); }); }
    function choosePreset() { showModal('Основа программы', `<div class="modal-body"><p class="help">Выбранная основа заменит только текущие шаблоны. История сохранится, старые шаблоны уйдут в архив.</p>${[['neo', 'Верх / низ', '4 шаблона, 3 занятия в неделю. Адаптация из нашего чата.'], ['vika', 'Программа из PDF katuabu', 'Вт — спина/руки; чт и вс — ягодицы/ноги/плечи. Числа источника сохранены.']].map(([id, title, text]) => `<button class="selection-card" data-action="apply-preset" data-preset="${id}"><span class="grow"><strong>${title}</strong><small>${text}</small></span>${ic('arrow')}</button>`).join('')}</div>`); }
    function applyPrograms(programs, schedule) { if (state.draft)
        throw Error('Заверши или отмени черновик перед сменой программы'); state.archivedPrograms.push({ date: new Date().toISOString(), programs: clone(state.programs) }); state.programs = clone(programs); state.schedule = clone(schedule); ui.program = 0; changed(); goto('program'); }
    ...
    function trackerEditor(category, id) { const existing = state.trackers.find(x => x.id === id), start = existing ? N.addDays(today(), 1) : today(), t = existing ? clone(existing) : { category, name: '', amount: '', notes: '', start, end: '', weekdays: [1, 2, 3, 4, 5, 6, 0], times: ['09:00'] }; formModal(existing ? 'Новая версия плана' : category === 'peptide' ? 'Добавить запись назначения' : 'Добавить план приёма', `<div class="notice ${category === 'peptide' ? 'warn' : ''}">${existing ? 'Прошлые отметки останутся прежними. Укажи дату, с которой меняется расписание.' : 'Названия, количество и время вводишь самостоятельно. Готовые схемы не подставляются.'}</div><div class="form-grid"><div class="form-full">${field('Название', 'name', t.name, 'text', 'required maxlength="140" placeholder="Название из твоего назначения"')}</div><div class="form-full">${field('Плановое количество и единицы', 'amount', t.amount, 'text', 'maxlength="240" placeholder="Запиши ровно как в назначении / инструкции"')}<p class="help" style="margin-top:6px">Это текстовая запись. Мг, мкг, мл и деления шприца не конвертируются и не считаются равнозначными.</p></div>${field(existing ? 'Изменить начиная с' : 'Дата начала', 'start', start, 'date', 'required')}${field('Дата окончания (необязательно)', 'end', t.end, 'date', '')}<div class="form-full"><div class="field-label" style="margin-bottom:9px">Дни приёма</div>${daySelect('weekdays', t.weekdays)}</div><div class="form-full">${field('Время: через запятую, максимум 6 приёмов', 'times', t.times.join(', '), 'text', 'required maxlength="80" placeholder="09:00, 20:00"')}</div><label class="form-full">Заметка / источник назначения<textarea name="notes" maxlength="4000">${h(t.notes)}</textarea></label>${category === 'peptide' ? `<div class="form-full">${check('ack', 'Понимаю: это личный журнал, не подбор курса или дозы', false)}</div>` : ''}</div>`, form => { const f = new FormData(form), date = String(f.get('start')), end = String(f.get('end')), times = String(f.get('times')).split(',').map(x => x.trim()), weekdays = f.getAll('weekdays').map(Number); if (!N.validDay(date) || (end && (!N.validDay(end) || end < date)))
        throw Error('Проверь начало и окончание'); if (!weekdays.length)
        throw Error('Выбери дни приёма'); if (times.length < 1 || times.length > 6 || new Set(times).size !== times.length || !times.every(x => /^([01]\d|2[0-3]):[0-5]\d$/.test(x)))
        throw Error('Время: 09:00, 20:00, без повторов'); if (category === 'peptide' && !f.has('ack'))
        throw Error('Подтверди, что это запись собственного назначения'); const patch = { category, name: String(f.get('name')).trim(), amount: String(f.get('amount')).trim(), notes: String(f.get('notes')), end, weekdays, times: times.sort(), timeZone: state.profile.timeZone }; if (!patch.name)
        throw Error('Нужно название'); if (existing) {
        if (date <= today())
            throw Error('Изменение уже действующего плана — не раньше завтра. Прошлое не переписываем.');
        N.reviseTracker(state, existing.id, patch, date);
    }
    else
        state.trackers.push({ id: uid(), ...patch, start: date }); changed(); close(); render(); toast('План записан. Приложение не назначало количества и дозировки.'); }); }
 ...
 lastExportRevision=state.revision;toast('Необязательная резервная JSON-копия зашифрована паролем текущего профиля.');
    ...
    function startTimer(e) { if (!state.draft || !e.restSeconds || ui.historyEdit)
        return; state.draft.timer = { name: e.name, endsAt: Date.now() + e.restSeconds * 1000, remaining: e.restSeconds * 1000, paused: false }; changed(); renderTimer(); }
    ...
    async function actions(b) {
        ...
        switch (a) {
            ...
            case 'apply-generated': {
                ...
                close();
                applyPrograms(generated, { mode: 'rotation', weekdays: ds, map: {}, nextIndex: 0 });
                break;
            }
            case 'start':
                begin(id);
            ...
            case 'add-set': {
                ...
                for (const side of e.unilateral ? ['left', 'right'] : ['both'])
                    e.logs.push({ id: uid(), number: n, side, weight: '', reps: '', rir: '', done: false, target: e.repsMin + '–' + e.repsMax });
                changed();
                render();
                break;
            }
            ...
            case 'finish': {
                ...
                confirm('Завершить тренировку?', `Отмечено ${t.done} из ${t.all} подходов. Неотмеченные значения сохранятся, но не будут считаться выполненными.`, () => { N.finishWorkout(state); changed(); ui.program = state.schedule.nextIndex; goto('history'); toast('Тренировка сохранена в истории.'); }, 'Завершить');
                break;
            }
            case 'discard':
    ...
    document.addEventListener('input', event => {
        ...
        if (el.id === 'exercise-search') {
            ...
            $('#exercise-options').innerHTML = pickerOptions(ui.picker.options.filter(x => N.normal(x.name + ' ' + x.group).includes(q)));
            return;
        }
        if (!state)
            return;
    ...
    document.addEventListener('change', event => { const el = event.target; if (el.id === 'import-file') {
        const f = el.files[0];
        el.value = '';
