...
(async () => {
    ...
    let ui = { tab: 'today', program: 0, mode: config.supabaseUrl && config.publishableKey ? 'cloud' : 'local', register: false, restoreMessage: '', date: N.dateKey(), month: N.dateKey().slice(0, 7), inBodyMetric: 'weight', inBodyImport: null, editor: null, picker: null, historyEdit: null, search: '', group: '' };
 ...
 ${last?`<section class="panel pad latest-workout"><div class="section-title"><h2>Последняя тренировка</h2><button class="btn ghost small" data-nav="history">Вся история ${ic('arrow')}</button></div><div class="row between"><div><h3>${h(last.title)}</h3><p class="help">${dateLabel(last.date)} · ${last.exercises.length} упражнений · ${N.totals(last).done} отмеченных подходов</p></div><button class="btn outline small" data-action="report" data-id="${h(last.id)}">${ic('copy')}Отчёт для тренера</button></div></section>`:''}`;
    ...
    const inBodyMetrics = [
        ['weight', 'Вес', 'кг'], ['skeletalMuscleMass', 'Скелетная мышечная масса', 'кг'], ['bodyFatMass', 'Жировая масса', 'кг'], ['bodyFatPercent', 'Доля жира', '%'], ['bmi', 'ИМТ', ''], ['visceralFatLevel', 'Висцеральный жир', 'ур.'], ['totalBodyWater', 'Вода', 'л'], ['protein', 'Белок', 'кг'], ['minerals', 'Минералы / кости', 'кг'], ['basalMetabolicRate', 'Основной обмен', 'ккал'], ['inBodyScore', 'Оценка InBody', '/100']
    ];
    ...
    function inBodyChart(records) {
        const def = inBodyMetrics.find(x => x[0] === ui.inBodyMetric) || inBodyMetrics[0], points = records.slice().sort((a, b) => a.measuredAt.localeCompare(b.measuredAt)).filter(x => Number.isFinite(x.metrics[def[0]]));
        if (points.length < 2) return '<p class="help">Для графика нужны минимум два замера с этим показателем.</p>';
        const values = points.map(x => x.metrics[def[0]]), min = Math.min(...values), max = Math.max(...values), span = max - min || 1;
        const xy = values.map((v, i) => [20 + i * 560 / Math.max(1, values.length - 1), 125 - (v - min) * 95 / span]);
        return `<svg class="inbody-chart" viewBox="0 0 600 155" role="img" aria-label="Динамика: ${h(def[1])}"><polyline points="${xy.map(p => p.join(',')).join(' ')}" fill="none" stroke="currentColor" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"/>${xy.map((p, i) => `<circle cx="${p[0]}" cy="${p[1]}" r="5"/><text x="${p[0]}" y="148" text-anchor="middle">${h(points[i].measuredAt.slice(5))}</text>`).join('')}</svg><p class="help">${h(def[1])}: ${metricText(values.at(-1), def[2])}; изменение от первого показанного замера ${values.at(-1) - values[0] > 0 ? '+' : ''}${String(Math.round((values.at(-1) - values[0]) * 100) / 100).replace('.', ',')} ${h(def[2])}.</p>`;
    }
    function renderInBody() {
        ...
        (latest ? `<div class="metrics inbody-metrics">${inBodyMetrics.slice(0, 6).map(([key, label, unit]) => { const value = latest.metrics[key], before = previous?.metrics[key], delta = Number.isFinite(value) && Number.isFinite(before) ? Math.round((value - before) * 100) / 100 : null; return `<section class="panel metric"><div class="metric-label">${h(label)}</div><strong>${metricText(value, unit)}</strong><small>${delta == null ? 'Нет сопоставимого прошлого замера' : 'К прошлому: ' + (delta > 0 ? '+' : '') + String(delta).replace('.', ',') + (unit ? ' ' + unit : '')}</small></section>`; }).join('')}</div><section class="panel pad"><div class="section-title"><h2>Динамика</h2><div class="row">${inBodyMetrics.slice(0, 4).map(([key, label]) => `<button class="btn small ${ui.inBodyMetric === key ? 'accent' : 'ghost'}" data-action="inbody-metric" data-metric="${key}">${h(label)}</button>`).join('')}</div></div>${inBodyChart(records)}</section>` : empty('Замеров пока нет', 'Импортируй PDF InBody или внеси показатели вручную. Исходный PDF остаётся на устройстве.', `<button class="btn primary" data-action="inbody-pdf">Импортировать PDF</button>`)) +
        `<section class="panel pad" style="margin-top:20px"><div class="section-title"><h2>История замеров</h2><span class="pill">${records.length}</span></div>${records.length ? records.map(x => `<article class="mini-item"><div><h3>${dateLabel(x.measuredAt)}</h3><p class="help">${h(x.source)} · импорт ${h((x.importedAt || '').slice(0, 10))}<br>Вес ${metricText(x.metrics.weight, 'кг')} · мышцы ${metricText(x.metrics.skeletalMuscleMass, 'кг')} · жир ${metricText(x.metrics.bodyFatPercent, '%')}</p></div><button class="btn outline small" data-action="inbody-edit" data-id="${h(x.id)}">${ic('edit')}Правка</button></article>`).join('') : '<p class="help">Пока пусто.</p>'}</section><div class="notice" style="margin-top:20px">Исходный PDF не загружается в публичные файлы или Storage: в профиль сохраняются только выбранные показатели, дата, источник и SHA-256 для защиты от дублей. Показатели InBody не меняют цели питания или анкету автоматически.</div>`;
    }
    function inBodyEditor(id) {
        const old = state.inBody.find(x => x.id === id), m = old?.metrics || {};
        formModal(old ? 'Исправить замер InBody' : 'Новый замер InBody', `<div class="notice">Правка влияет только на этот замер. Профиль, калории и программа не меняются.</div><div class="form-grid">${field('Дата измерения', 'measuredAt', old?.measuredAt || today(), 'date', 'required')}${inBodyMetrics.map(([key, label, unit]) => field(label + (unit ? ', ' + unit : ''), key, m[key] ?? '', 'number', 'min="0" max="100000" step="0.01"')).join('')}<label class="form-full">Источник<textarea name="source" maxlength="240">${h(old?.source || 'Ручной ввод')}</textarea></label></div>`, async form => {
            ...
            for (const [key] of inBodyMetrics) { const raw = String(f.get(key) || '').trim(); if (raw !== '') { const value = Number(raw.replace(',', '.')); if (!Number.isFinite(value) || value < 0) throw Error('Проверь числовые показатели'); metrics[key] = value; } }
            if (!Object.keys(metrics).length) throw Error('Введи хотя бы один показатель');
            const source = String(f.get('source') || 'Ручной ввод').trim() || 'Ручной ввод';
            const record = { id: old?.id || uid(), measuredAt, importedAt: old?.importedAt || new Date().toISOString(), source, sourceHash: old?.sourceHash || await sha256Hex(new TextEncoder().encode('manual|' + measuredAt + '|' + JSON.stringify(metrics) + '|' + Date.now())), metrics };
    ...
    async function parseInBodyPdf(file) {
        ...
        const pdf = await getDocument({ data: new Uint8Array(data) }).promise; let text = '';
        for (let pageNo = 1; pageNo <= pdf.numPages; pageNo++) { const page = await pdf.getPage(pageNo), content = await page.getTextContent(); text += ' ' + content.items.map(x => x.str).join(' '); }
        const { measuredAt, metrics } = parseInBodyText(text);
        return { id: uid(), measuredAt, importedAt: new Date().toISOString(), source: 'InBody PDF · локальный импорт', sourceHash, metrics };
    }
    async function previewInBodyPdf(file) {
        const record = await parseInBodyPdf(file); ui.inBodyImport = record;
        showModal('Проверь импорт InBody', `<div class="modal-body"><div class="notice">PDF обработан только в этом браузере. Оригинал не загружается и не сохраняется.</div><h3>Дата: ${h(record.measuredAt)}</h3><div class="grid2" style="margin-top:16px">${inBodyMetrics.filter(([key]) => record.metrics[key] != null).map(([key, label, unit]) => `<div class="mini-item"><span class="help">${h(label)}</span><strong>${metricText(record.metrics[key], unit)}</strong></div>`).join('')}</div></div>`, `<div class="modal-foot"><button class="btn outline" data-action="close">Отмена</button><button class="btn primary" data-action="inbody-save-import">Сохранить без изменения профиля</button></div>`);
    }
    ...
    function trackerEditor(category, id) {
        const existing = state.trackers.find(x => x.id === id);
        const start = existing ? N.addDays(today(), 1) : today();
        const t = existing ? clone(existing) : { category, name: '', form: 'other', unit: '', perDose: '', substancePerUnit: '', foodRelation: 'any', notes: '', start, end: '', weekdays: [1, 2, 3, 4, 5, 6, 0], doses: [{ time: '09:00', amount: '' }] };
        const dosesText = (t.doses?.length ? t.doses : (t.times || ['09:00']).map(time => ({ time, amount: t.perDose || t.amount || '' }))).map(d => d.time + ' | ' + (d.amount || '')).join('\n');
        const suggestions = category === 'supplement' ? '<datalist id="supplement-names"><option value="Витамин D"><option value="Магний"><option value="Омега-3"><option value="Креатин"><option value="Протеин"></datalist>' : '';
        formModal(existing ? 'Новая версия плана' : category === 'peptide' ? 'Добавить запись назначения' : 'Добавить план приёма',
          `<div class="notice ${category === 'peptide' ? 'warn' : ''}">${existing ? 'Прошлые отметки останутся прежними. Укажи дату, с которой меняется расписание.' : 'Названия, количество и время вводишь самостоятельно. Готовые схемы не подставляются.'}</div>${suggestions}<div class="form-grid">
          <div class="form-full">${field('Название', 'name', t.name, 'text', 'required maxlength="140" list="supplement-names" placeholder="Название из назначения или упаковки"')}</div>
          ${select('Форма', 'form', t.form || 'other', { capsule: 'Капсула', tablet: 'Таблетка', powder: 'Порошок', liquid: 'Жидкость', injection: 'Инъекция', other: 'Другое' })}
          ${field('Единица учёта', 'unit', t.unit || '', 'text', 'maxlength="40" placeholder="капс., мл, г…')}
          ${field('Количество за один приём', 'perDose', t.perDose || t.amount || '', 'text', 'maxlength="80" placeholder="1 капс. / 5 мл')}
          ${field('Вещества в единице (необязательно)', 'substancePerUnit', t.substancePerUnit || '', 'text', 'maxlength="80" placeholder="500 мг')}
          ${select('Связь с едой', 'foodRelation', t.foodRelation || 'any', { any: 'Не указано', 'with-food': 'Во время еды', 'after-food': 'После еды', 'before-food': 'До еды', empty: 'Натощак' })}
          ${field(existing ? 'Изменить начиная с' : 'Дата начала', 'start', start, 'date', 'required')}
          ${field('Дата окончания (необязательно)', 'end', t.end, 'date', '')}
          <div class="form-full"><div class="field-label" style="margin-bottom:9px">Дни приёма</div>${daySelect('weekdays', t.weekdays)}</div>
          <label class="form-full">Время и количество — одна строка на приём<textarea name="doses" required maxlength="600" placeholder="09:00 | 1 капс.&#10;20:00 | 2 капс.">${h(dosesText)}</textarea><span class="help">Формат: ЧЧ:ММ | количество. Можно указать разное количество в разное время.</span></label>
          <label class="form-full">Заметка / источник назначения<textarea name="notes" maxlength="4000">${h(t.notes)}</textarea></label>
          ${category === 'peptide' ? `<div class="form-full">${check('ack', 'Понимаю: это личный журнал, не подбор курса или дозы', false)}</div>` : ''}</div>`,
          form => {
            const f = new FormData(form), date = String(f.get('start')), end = String(f.get('end')), weekdays = f.getAll('weekdays').map(Number);
            const doses = String(f.get('doses')).split(/\r?\n/).map(x => x.trim()).filter(Boolean).map(line => {
                const [time, ...rest] = line.split('|');
                return { time: String(time || '').trim(), amount: rest.join('|').trim() };
            });
            ...
            if (doses.length < 1 || doses.length > 6 || new Set(doses.map(d => d.time)).size !== doses.length || !doses.every(d => /^([01]\d|2[0-3]):[0-5]\d$/.test(d.time))) throw Error('Каждая строка: 09:00 | количество; время без повторов');
            if (category === 'peptide' && !f.has('ack')) throw Error('Подтверди, что это запись собственного назначения');
            const perDose = String(f.get('perDose')).trim();
            const patch = { category, name: String(f.get('name')).trim(), form: String(f.get('form')), unit: String(f.get('unit')).trim(), perDose, substancePerUnit: String(f.get('substancePerUnit')).trim(), foodRelation: String(f.get('foodRelation')), amount: perDose, notes: String(f.get('notes')), end, weekdays, doses: doses.map(d => ({ ...d, amount: d.amount || perDose })), times: doses.map(d => d.time).sort(), timeZone: state.profile.timeZone };
            patch.doses.sort((a, b) => a.time.localeCompare(b.time));
            if (!patch.name) throw Error('Нужно название');
            if (existing) {
                if (date <= today()) throw Error('Изменение уже действующего плана — не раньше завтра. Прошлое не переписываем.');
                N.reviseTracker(state, existing.id, patch, date);
            } else state.trackers.push({ id: uid(), ...patch, start: date });
