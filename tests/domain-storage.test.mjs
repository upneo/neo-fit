...
test('CAS migration uses one PT409 application conflict and never SQLSTATE 40001',()=>{
 const sql=fs.readFileSync(new URL('../supabase/migrations/202609170004_fix_diary_cas_conflict.sql',import.meta.url),'utf8');
 assert.match(sql,/sqlstate 'PT409'/);assert(!sql.includes("errcode='40001'"));
 const client=fs.readFileSync(new URL('../src/lib/storage.js',import.meta.url),'utf8');assert(client.includes("error.code === 'PT409'"));
});
test('deleting a plan removes future occurrences but retains recorded intake history',()=>{
 const s=base(),t=tracker(s),past=N.occurrences(s,'2026-01-06')[0];N.recordIntake(s,past,'taken','','08:02','1');
 N.deleteTracker(s,t.id);assert.equal(s.trackers.length,0);assert.equal(N.occurrences(s,'2026-01-07').length,0);
 const retained=N.occurrences(s,'2026-01-06');assert.equal(retained.length,1);assert.equal(retained[0].status,'taken');assert(retained[0].legacy);
});
test('manual past workout updates queue only when it is unambiguous and selected',()=>{
 const s=base(),id=s.schedule.queue[0],before=s.schedule.nextIndex;const d=N.startPastWorkout(s,id,'2026-01-05');
 Object.assign(d.exercises[0].logs[0],{weight:'20',reps:'10',done:true});assert.equal(d.queueEligible,true);d.queueIncluded=true;N.finishWorkout(s);
 assert.equal(s.history[0].addedManually,true);assert.equal(s.schedule.nextIndex,1);assert.notEqual(s.schedule.nextIndex,before);
 const off=base(),d2=N.startPastWorkout(off,off.schedule.queue[0],'2026-01-05');Object.assign(d2.exercises[0].logs[0],{weight:'20',reps:'10',done:true});d2.queueIncluded=false;N.finishWorkout(off);assert.equal(off.schedule.nextIndex,0);
});
